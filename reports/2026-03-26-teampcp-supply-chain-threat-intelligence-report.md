...
**Classification:** TLP:WHITE
**Sources Analyzed:** 20
**Campaign Timeline:** December 2025 – March 2026
...
**kamikaze.sh** — The initial credential harvester delivered via compromised Trivy GitHub Actions. This shell script evolved through three versions: v1 performed basic credential exfiltration; v2 added GitHub runner process memory scraping (reading `/proc/<pid>/mem` to bypass GitHub secret masking and extract plaintext tokens); v3 introduced a pull method to download secondary payloads. All versions exfiltrated data to typosquatted domains using HTTP POST with AES-256-CBC encryption wrapped in a 4096-bit RSA public key.
...
**Primary C2 Domains (Typosquatted):** Each attack wave used a vendor-themed typosquatted domain for exfiltration, providing plausible-looking traffic patterns: `scan.aquasecurtiy\[.]org` (Trivy), `checkmarx\[.]zone` (KICS), `models.litellm\[.]cloud` (LiteLLM).
...
**GitHub-Based Fallback:** If primary C2 fails, the malware uses the victim's own GITHUB\_TOKEN to create hidden repositories (naming pattern: `tpcp-docs-\*`, `docs-tpcp`) for fallback data exfiltration, making detection more challenging since the traffic originates from legitimate GitHub API endpoints.

**Dedicated IP Infrastructure:** Eight C2 IP addresses have been consistently observed across the campaign (see IoC list below).
...
Organizations should implement detection across multiple layers:

**CI/CD Pipeline Monitoring:** Monitor for unexpected modifications to GitHub Actions workflows, especially those using `pull\_request\_target`. Alert on commits from unrecognized bot accounts. Validate SHA-pinned references for all GitHub Actions rather than relying on mutable version tags.

**Package Registry Monitoring:** Monitor for unexpected version publications of critical dependencies (Trivy, LiteLLM, Checkmarx, Telnyx). Implement SBOM verification and enforce allowlisted package versions. Alert on `.pth` file creation in Python site-packages directories.

**Network Detection:** Block or alert on connections to the IoC domains and IPs listed below. Monitor for DNS resolution of trycloudflare.com subdomains from CI/CD runners. Detect HTTP POST requests with large encrypted payloads to unrecognized endpoints. Monitor for connections to ICP canister domains (`icp0.io`).

**Kubernetes Detection:** Alert on creation of unexpected DaemonSets in `kube-system` namespace, especially named `host-provisioner-iran` or `host-provisioner-std`. Detect containers named `kamikaze` or `provisioner` mounting host root filesystems. Monitor for privileged container creation via Docker API (port 2375).

**Host-Level Detection:** Monitor for creation of systemd services masquerading as PostgreSQL utilities (`pgmon`, `pgmonitor`, `pglog`, `pg\_state`, `internal-monitor`). Detect file writes to `/var/lib/pgmon/`, `/var/lib/svc\_internal/`, or `/host/root/.config/sysmon/`. Alert on SSH connections with `StrictHostKeyChecking=no` combined with lateral movement patterns. Monitor for WAV file downloads followed by process execution.
...
TeamPCP represents a new paradigm in cloud-native threat actors: one that combines industrial-scale automation, supply chain expertise, and an evolving toolkit to attack the trust foundations of modern software development. In under four months, the group evolved from opportunistic exploitation of exposed Docker APIs to a coordinated supply chain campaign that compromised five major vendor ecosystems, deployed a blockchain-resilient worm, integrated a geopolitically targeted wiper, and partnered with a ransomware-as-a-service operation — all from a single unrevoked CI credential.

The campaign exposes critical systemic weaknesses: incomplete credential rotation, implicit trust in security tooling, mutable version references in CI/CD workflows, and the lack of behavioral monitoring in software supply chains. Cryptographic integrity checks alone failed against attacks using legitimate, compromised credentials.

Organizations must urgently review their software bills of materials for affected packages, enforce immutable SHA-pinned workflow references, implement strict credential rotation with verification, deploy behavioral anomaly detection across CI/CD environments, and monitor for all listed indicators of compromise. The convergence of supply chain compromise, credential theft, worm propagation, wiper deployment, and ransomware partnership makes TeamPCP one of the most operationally dangerous threat actors active today.
...
|-|-|-|
|`23.142.184\[.]129`|IPv4|TeamPCP C2 server for exfiltration and remote access|
|`45.148.10\[.]212`|IPv4|TeamPCP C2 server for data exfiltration|
|`63.251.162\[.]11`|IPv4|TeamPCP C2 server|
|`83.142.209\[.]11`|IPv4|TeamPCP C2 server|
|`83.142.209\[.]203`|IPv4|TeamPCP C2 server|
|`195.5.171\[.]242`|IPv4|TeamPCP C2 server|
|`209.34.235\[.]18`|IPv4|TeamPCP C2 server|
|`212.71.124\[.]188`|IPv4|TeamPCP C2 server|

### 6.2 Domains \& URLs (C2 / Exfiltration)

|IoC|Type|Description|
|-|-|-|
|`scan.aquasecurtiy\[.]org`|Domain|Typosquatted primary C2 — Trivy exfiltration|
|`checkmarx\[.]zone`|Domain|Typosquatted primary C2 — Checkmarx KICS exfiltration|
|`models.litellm\[.]cloud`|Domain|Typosquatted primary C2 — LiteLLM exfiltration|
|`tdtqy-oyaaa-aaaae-af2dq-cai.raw.icp0\[.]io`|Domain|ICP Canister decentralized backup C2|
|`championships-peoples-point-cassette.trycloudflare\[.]com`|Domain|Cloudflare tunnel C2|
|`create-sensitivity-grad-sequence.trycloudflare\[.]com`|Domain|Cloudflare tunnel C2|
|`investigation-launches-hearings-copying.trycloudflare\[.]com`|Domain|Cloudflare tunnel C2|
|`plug-tab-protective-relay.trycloudflare\[.]com`|Domain|Cloudflare tunnel C2|
|`souls-entire-defined-routes.trycloudflare\[.]com`|Domain|Cloudflare tunnel C2|
...
|IoC|Type|Description|
|-|-|-|
|`kamikaze.sh`|Filename|Initial credential harvester script|
|`kube.py`|Filename|Worm/wiper payload for Kubernetes|
|`prop.py`|Filename|Malicious Python payload|
|`proxy\_server.py`|Filename|Malicious Python payload|
|`tpcp.tar.gz`|Filename|Malicious archive for payload delivery|
|`/host/root/.config/sysmon/sysmon.py`|Path|Persistence dropper location|
|`/var/lib/pgmon/pgmon.py`|Path|CanisterWorm backdoor (PostgreSQL disguise)|
|`/var/lib/svc\_internal/runner.py`|Path|Wiper/backdoor runner script|
|`/tmp/pglog`|Path|Temporary file for malware staging|
|`/tmp/.pg\_state`|Path|Temporary file for malware staging|
|`/etc/systemd/system/internal-monitor.service`|Path|Malicious systemd persistence|
|`/etc/systemd/system/pgmonitor.service`|Path|Malicious systemd persistence|
...
|Container name: `kamikaze`|Wiper container within DaemonSet|
|Container name: `provisioner`|Backdoor container within DaemonSet|
...
|-|-|-|-|
|T1195|Supply Chain Compromise|Initial Access|Systematic compromise of trusted OSS security tools (Trivy, KICS, LiteLLM, Telnyx) to gain access to downstream consumers|
...
|T1036.005|Masquerading: Match Legitimate Name or Location|Defense Evasion|Disguising malware as PostgreSQL utilities (pgmon, pglog, pg\_state) and systemd services|
...
|T1552.001|Unsecured Credentials: Credentials in Files|Credential Access|Sweeping .env files, AWS/Azure/GCP config directories, SSH keys for credential harvesting|
...
|T1558.001|Steal or Forge Kerberos Tickets: Golden Ticket|Credential Access|Using victim's GITHUB\_TOKEN to create hidden repositories as fallback exfiltration channel|
...
|T1021.007|Remote Services: Kubernetes API|Lateral Movement|Scanning exposed Docker APIs (port 2375), Kubernetes API exploitation, SSH key harvesting for lateral spread|
...
|T1573.002|Encrypted Channel: Asymmetric Cryptography|Command and Control|AES-256-CBC session key wrapped in 4096-bit RSA public key for encrypted C2 traffic|
|T1008|Fallback Channels|Command and Control|ICP canister backup C2, Cloudflare tunnels, victim GitHub repos as fallback channels|
|T1486|Data Encrypted for Impact|Impact|Ransomware deployment via Vect partnership for extortion at scale|
...
Data extracted from 20 indexed reports spanning 12 distinct sources
